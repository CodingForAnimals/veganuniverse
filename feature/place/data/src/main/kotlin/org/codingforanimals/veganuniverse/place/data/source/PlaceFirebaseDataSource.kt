...
import org.codingforanimals.veganuniverse.place.data.paging.PlaceListingPagingSource
import org.codingforanimals.veganuniverse.place.data.paging.PlacePagingSource
...
internal class PlaceFirebaseDataSource(
    ...
    override suspend fun editPlace(placeId: String, userId: String, suggestion: String) {
        placesSuggestionsReference.child(placeId).child(userId).setValue(suggestion).await()
    }
    ...
    override suspend fun validatePlace(id: String) {
        coroutineScope {
            val validateCardDeferred =
                cardsReference.child(id).child(VALIDATED_RTDB_FIELD).setValue(true).asDeferred()
            val validatePlaceDeferred = placesCollection.document(id).update(
                VALIDATED_FIRESTORE_FIELD, true
            ).asDeferred()
    ...
    companion object {
        ...
        internal const val PLACES_SUGGESTIONS = "content/places/suggestions"
