...
class SubmitRecipe(
    ...
    suspend operator fun invoke(recipeForm: RecipeForm): Result<String> = runCatching {
        ...
        recipeRepository.insertRecipe(recipeFormAsModel, recipeForm.imageModel).also {
            profileRecipeUseCases.addContribution(it)
        }
    }.onFailure {
        Log.e("SubmitRecipe", "Error submitting recipe", it)
