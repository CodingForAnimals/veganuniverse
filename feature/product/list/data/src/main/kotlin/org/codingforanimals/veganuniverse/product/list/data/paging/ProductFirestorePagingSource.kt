package org.codingforanimals.veganuniverse.product.list.data.paging
...
) : PagingSource<QuerySnapshot, ProductEntity>() {

    override fun getRefreshKey(state: PagingState<QuerySnapshot, ProductEntity>): QuerySnapshot? {
        return null
    }

    override suspend fun load(params: LoadParams<QuerySnapshot>): LoadResult<QuerySnapshot, ProductEntity> {
        return try {
            ...
            LoadResult.Page(
                data = currentPage.toObjects(ProductEntity::class.java),
                prevKey = params.key,
                nextKey = nextPage
            )
        } catch (e: Throwable) {
